...
uint32_t cipInstructUser(int argc, char * argv[]) {
  ...
  if (executeAll) {
    ...
    for (int step = 0; step < steps; step++) {
      sprintf(buf1,"cipInstructUser: for...loop. step %d line# (%d) ...\n",step,__LINE__);
      ecmdOutput(buf1);
      ...
      target.cageState = target.nodeState = target.slotState = target.posState = ECMD_TARGET_FIELD_WILDCARD;
      if (validChipFound == true) {
        sprintf(buf1,"cipInstructUser: Valid chip found [CU:%s] line# (%d)...\n",validTarget.chipUnitType.c_str(),__LINE__);
        ecmdOutput(buf1);
      ...
      while (ecmdLooperNext(target, looperData)) {
        sprintf(buf1,"cipInstructUser: Looping through the cores in forward order (core:%d)(Thread:%d) line# (%d)   ...\n",target.chipUnitNum, target.thread,__LINE__);
        ecmdOutput(buf1);
        ...
        if (target.chipUnitType == "occ") {
          ...
          subTarget.threadState = ECMD_TARGET_FIELD_VALID;
        ...
        loopMode = ECMD_CONFIG_LOOP;
        sprintf(buf1,"cipInstructUser: loopMode (%d) line# (%d)  ...\n",loopMode,__LINE__);
        ecmdOutput(buf1);

        rc = ecmdLooperInit(subTarget, ECMD_SELECTED_TARGETS_LOOP, subLooperData, loopMode);
        sprintf(buf1,"cipInstructUser: Looper Init target (Thread:%d) line# (%d) ...\n",subTarget.thread, __LINE__);
        ecmdOutput(buf1);
        if (rc) return rc;

        while (ecmdLooperNext(subTarget, subLooperData) && (!coeRc || coeMode)) {
          sprintf(buf1,"cipInstructUser: Looper Next target (Thread:%d) line# (%d) ...\n",subTarget.thread,__LINE__);
          ecmdOutput(buf1);
          if (firstThreadLoop) {
            if (!strcasecmp(argv[0 + argOffset],"start") || !strcasecmp(argv[0 + argOffset],"sreset")) {
          ...
          if (rc) {
            ...
            sprintf(buf1,"cipInstructUser: argument passed (%s) successfull line# (%d)...\n",argv[0 + argOffset],__LINE__);
            ecmdOutput(buf1);
            validPosFound = true;     
            threadFound = true;
          }
